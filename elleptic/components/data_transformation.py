...
from elleptic.config import in_col,TARGET_COLUMN
...
class DataTransformation:
    ...
    def get_data_transformer_object(cls):
        try:            
            ...
            # transformer_steps = [(f"scaler_{col}", RobustScaler()) for col in cls.columns if col not in exclude_columns]
            # transformer = ColumnTransformer(transformer_steps)
            # return transformer
            return pipeline
        except Exception as e:
            raise EllepticException(e, sys)
